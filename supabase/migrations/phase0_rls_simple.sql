...
-- Drop any existing policies first (in case of previous failed attempts)
...
CREATE POLICY "service_full_cases" ON cases FOR ALL USING (
  COALESCE(
...
CREATE POLICY "service_full_entities" ON entities FOR ALL USING (
  COALESCE(
    current_setting('request.jwt.claims', true)::json->>'role',
    ''
  ) = 'service_role'
...
CREATE POLICY "service_full_actions" ON actions FOR ALL USING (
  COALESCE(
    current_setting('request.jwt.claims', true)::json->>'role',
    ''
  ) = 'service_role'
...
CREATE POLICY "service_full_reputation" ON reputation FOR ALL USING (
  COALESCE(
    current_setting('request.jwt.claims', true)::json->>'role',
    ''
  ) = 'service_role'
...
CREATE POLICY "service_full_submissions" ON triple_submissions FOR ALL USING (
  COALESCE(
    current_setting('request.jwt.claims', true)::json->>'role',
    ''
  ) = 'service_role'
...
CREATE POLICY "service_full_cards" ON user_cards FOR ALL USING (
  COALESCE(
    current_setting('request.jwt.claims', true)::json->>'role',
    ''
  ) = 'service_role'
